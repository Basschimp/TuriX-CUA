import os, sys, json, logging, argparse, asyncio
...
from src.controller.service_win import Controller  # Windows controller
...
HOTKEY_ALIASES = {
    "win": "<win>",
    "windows": "<win>",
    "command": "<win>",  # Map Mac command to Windows key
    "cmd": "<win>",
    "shift": "<shift>",
    "control": "<ctrl>",
    "ctrl": "<ctrl>",
...
def register_force_stop_hotkey(
    ...
    """Register a global hotkey for force-stopping the agent."""
...
def build_llm(cfg: dict, *, enable_thinking: bool | None = None):
    ...
    if provider == "hermes":
        # Hermes models via OpenRouter or other providers
        return build_openai_compatible_llm(
            model_name=model_name,
            api_key=api_key,
            base_url=base_url or "https://openrouter.ai/api/v1",
